...
<<<<<<< HEAD
		...
		# Initialisations de la liste des voisins de chaque planète (sert à la génération du dictionnaire).
		for pla in carte.liste_planetes:
			lst_vois = []

			for are in carte.liste_aretes:
				_lst_vois = []

				if pla.identifiant in are.extremites:
					_lst_vois.append(are.distance)
					_lst_vois.append(are.getAutreExtremite(pla.identifiant))

					pla.liste_voisins.append(_lst_vois)
...
=======
        ...
        carte.threads.append(threading.Thread(target=maMap.boucle_principale, name="interface", args=(carte,)))
        carte.threads[0].start()
>>>>>>> bbac8f60091dfa4c79224d4dd2304f5eb82d853a
